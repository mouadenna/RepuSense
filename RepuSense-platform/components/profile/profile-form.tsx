...
export function ProfileForm() {
  ...
  const [brands, setBrands] = useState<string[]>(["MarqueA", "ProduitB", "ServiceC"])
  const [newBrand, setNewBrand] = useState("")

  // État pour les employés clés
  const [employees, setEmployees] = useState<{ name: string; role: string }[]>([
    { name: "Jean Dupont", role: "CEO" },
    { name: "Marie Martin", role: "Community Manager" },
    { name: "Pierre Leroy", role: "Directeur Marketing" },
  ])
  ...
  const [socialMedia, setSocialMedia] = useState<{ platform: string; handle: string }[]>([
    { platform: "Twitter", handle: "@entreprise" },
    { platform: "Facebook", handle: "entrepriseofficiel" },
    { platform: "LinkedIn", handle: "entreprise-sas" },
  ])
  const [newSocialPlatform, setNewSocialPlatform] = useState("")
  const [newSocialHandle, setNewSocialHandle] = useState("")

  // État pour les hashtags
  const [hashtags, setHashtags] = useState<string[]>(["#entreprise", "#innovation", "#tech"])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({
        title: "Erreur",
        description: "Une erreur est survenue lors de la mise à jour du profil. Veuillez réessayer.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
      ...
      <Tabs defaultValue="company" className="w-full">
        ...
        <TabsContent value="company" className="space-y-4 pt-4">
          <div className="space-y-2">
            ...
            <Input id="companyName" defaultValue="Entreprise SAS" required disabled={isLoading} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="industry">Secteur d'activité</Label>
            <Input id="industry" defaultValue="Commerce électronique" required disabled={isLoading} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="website">Site web</Label>
            <Input id="website" type="url" defaultValue="https://www.entreprise.com" required disabled={isLoading} />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="description"
              defaultValue="Entreprise leader dans le secteur du commerce électronique, spécialisée dans la vente de produits innovants."
              rows={4}
              disabled={isLoading}
            />
          </div>
